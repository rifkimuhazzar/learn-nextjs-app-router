...
function initDb() {
  ...
  const stmt = db.prepare<any[], any>("SELECT COUNT(*) AS count FROM users");
...
export async function getPosts(maxNumber?: any) {
  let limitClause = "";
...
export async function storePost(post: any) {
  const stmt = db.prepare(`
